...
__z Confidence Interval__ is based on CLT.  
$$Est \pm ZQ \times SE_{Est}$$  

__Gosset's $t$ distribution and $t$ confidence intervals__  
$t$ distribution is for small samples.  
...
Invented by William Gosset, with a thicker tails than the normal. It is indexed by a degrees of freedom; gets more like a standard normal as df gets larger. It assumes that the underlying data are iid.   
...
`df`: degrees of freedom, positive, maybe non-integer. $df = Inf$ is allowed.  
`ncp`: non-centrality parameter delta; currently except for $rt()$, only for $abs(ncp) \le 37.62$. If omitted, use the central $t$ distribution.  
